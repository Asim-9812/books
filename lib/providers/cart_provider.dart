...
class CrudNotifier extends StateNotifier<CrudState> {
  ...
  Future<void> delProduct({
    ...
    state = state.copyWith(isLoad: true, errorMessage: '', isSuccess: false);
    final response = await CrudService.delProduct(postId: postId, imageId: imageId, token: token);
    response.fold((l) {
      state = state.copyWith(isLoad: false, errorMessage: l, isSuccess: false);
    }, (r) {
      state = state.copyWith(isLoad: false, errorMessage: '', isSuccess: true);
  ...
  Future<void> updateProduct({
    ...
    state = state.copyWith(isLoad: true, errorMessage: '', isSuccess: false);
    final response = await CrudService.updateProduct(title: title, detail: detail, price: price, postId: postId, image: image, imageId: imageId, token: token);
    response.fold((l) {
      state = state.copyWith(isLoad: false, errorMessage: l, isSuccess: false);
    }, (r) {
      state = state.copyWith(isLoad: false, errorMessage: '', isSuccess: true);
